...
def mc_error_analysis(windows, data):
    ...
    for i in range(num_mc_runs):
        ...
        for w in windows:
            ...
            fw.hist[:, 0] = data[:, 0]
            rand = np.random.random(int(np.ceil(fw.num_points / int(fw.correl_time))))
            diffs = np.abs(np.add.outer(-rand, w.hist[:, 2]))
            indices = np.argmin(diffs, axis=1) + w.min
            fw.hist[:, 1] += np.bincount(indices, minlength=fw.hist[:, 1].size)
...
def blockAverage(datastream, isplot=False, maxBlockSize=0):
    ...
    for blockSize in range(minBlockSize, maxBlockSize):
        ...
        obsProp = np.mean(
            np.reshape(datastream[: Nobs - resid], (Nblock, blockSize)), axis=1
        )
...
def block_analysis(windows, data):
    ...
    for i in range(num_windows):
        v, blockVar, blockMean = blockAverage(windows[i].traj, maxBlockSize=50)
        xbar[i] = blockMean[-1]
        try:
...
def split_analysis(windows, data):
    ...
    for i in range(num_split):
        ...
        for w in windows:
            ...
            splitw.hist[:, 1] = np.histogram(
                w.traj[rangemin:rangemax],
                bins=num_bins,
...
def consistency_tests(windows, data):
    ...
    for i in range(1, num_windows):
        ...
        wl = np.array(
            [
                calc_bias(windows[i - 1].spring, windows[i - 1].loc, data[l, 0])
                for l in range(num_bins)
            ]
        )
        wr = np.array(
            [
                calc_bias(windows[i].spring, windows[i].loc, data[l, 0])
                for l in range(num_bins)
            ]
        )

        wl_star = np.array(
            [calc_bias(windows[i - 1].spring, r, data[l, 0]) for l in range(num_bins)]
        )
        wr_star = np.array(
            [calc_bias(windows[i].spring, r, data[l, 0]) for l in range(num_bins)]
        )

        num = np.array(
            [
                windows[i - 1].p[l] * np.exp((wl[l] - wl_star[l]) / kT)
                for l in range(num_bins)
            ]
        )
        denom = sum(
        ...
        num = np.array(
            [
                windows[i].p[l] * np.exp((wr[l] - wr_star[l]) / kT)
                for l in range(num_bins)
            ]
        )
        denom = sum(
            windows[i].p[l] * np.exp((wr[l] - wr_star[l]) / kT) for l in range(num_bins)
        )
        pr_star = num / denom

        maxval = max(np.sum(abs(pl_star - pr_star)[:i]) for i in range(num_bins))
        phi[i] = maxval
